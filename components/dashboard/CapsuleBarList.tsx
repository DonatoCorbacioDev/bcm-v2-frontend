interface CapsuleBarRow {
  label: string;
  value: number;
  formattedValue: string;
}

const CHART_COLORS = [
  "var(--chart-1)",
  "var(--chart-2)",
  "var(--chart-3)",
  "var(--chart-4)",
  "var(--chart-5)",
];

/** Minimal label + pill-track comparison, matching the design mockup's
 * "Valore per area di business" — no charting library, just proportional fills.
 * Bars are colored by rank (highest value first) from the shared chart
 * palette, independent of the order rows are passed in. */
export function CapsuleBarList({ rows }: { readonly rows: CapsuleBarRow[] }) {
  const max = Math.max(...rows.map((r) => r.value), 1);
  const rankByLabel = new Map(
    [...rows].sort((a, b) => b.value - a.value).map((r, i) => [r.label, i])
  );

  return (
    <div className="space-y-3">
      {rows.map((row) => {
        const rank = rankByLabel.get(row.label) ?? 0;
        const color = CHART_COLORS[rank % CHART_COLORS.length];
        return (
          <div key={row.label} className="flex items-center gap-3">
            <span
              className="w-[140px] shrink-0 truncate text-sm text-secondary-foreground"
              title={row.label}
            >
              {row.label}
            </span>
            <div className="h-[13px] flex-1 overflow-hidden rounded-full bg-muted">
              <div
                className="h-full rounded-full transition-[width] duration-500 ease-out"
                style={{ width: `${(row.value / max) * 100}%`, backgroundColor: color }}
              />
            </div>
            <span className="w-16 shrink-0 text-right font-mono text-sm tabular-nums text-foreground">
              {row.formattedValue}
            </span>
          </div>
        );
      })}
    </div>
  );
}
